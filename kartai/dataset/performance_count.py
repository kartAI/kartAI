...
def get_performance_count_for_detected_buildings(all_predicted_buildings, predictions_path, true_labels,new_buildings_fasit, CRS_prosjektomrade, model_name, performance_output_dir, region_name):
    ...
    correctly_detected_new_buildings = get_correctly_detected_new_buildings(
       new_buildings_fasit, correctly_detected_buildings,  predictions_path, CRS_prosjektomrade)

    wrongly_detected_buildings = get_wrongly_detected_buildings(
        all_predicted_buildings, true_labels, predictions_path, CRS_prosjektomrade)
    ...
    all_predicted_buildings = gp.clip(
        all_predicted_buildings, area_gdf)
...
def get_wrongly_detected_buildings(all_predictions, true_labels_dataset, predictions_path, crs):
...
def get_correctly_detected_new_buildings(new_buildings_fasit, true_predicted_buildings, predictions_path, crs):
    ...
    true_detected_buildings = true_predicted_buildings.loc[true_predicted_buildings.intersects(new_buildings_fasit.unary_union)]
...
def get_correctly_detected_buildings(true_labels, all_predicted_buildings_dataset):
    ...
    #Find all buildings that intersect with an existing building
    true_predictions = all_predicted_buildings_dataset.loc[all_predicted_buildings_dataset.intersects(true_labels.unary_union)].reset_index(drop=True)

    #Clip the true_labels to the predictions, in order to keep the label_id column
    true_predictions = gp.overlay(true_predictions, true_labels, how='intersection')
...
def set_overlapping_label_id(predictions, true_labels):
  predictions["label_id"] = predictions.map(lambda pred: true_labels.intersects(pred.unary_union)["label_id"])
