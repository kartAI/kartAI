...
def save_dataset(data, filename, output_dir, modelname, save_to):
    if save_to == "azure":
        upload_data_to_azure(
            data,
            modelname + "/" + filename,
            env.get_env_variable("results_datasets_container_name"),
        )
    else:
...
def run_ml_predictions(
    ...
    region_name: str,
    projection: str,
    input_model_subfolder: str = None,
    dataset_path_to_predict: str = None,
    config: str = None,
    geom: ogr.Geometry = None,
    skip_data_fetching: bool = False,
    tupple_data: bool = False,
    ...
    dataset_path_to_predict = (
        dataset_path_to_predict
        if dataset_path_to_predict
        else get_dataset_to_predict_dir(region_name)
    )
    ...
    raster_output_dir = get_raster_predictions_dir(region_name, input_model_name)
    ...
    splits = (
        ...
        if num_predictions % batch_size == 0
        else (num_predictions // batch_size) + 1
    )
    ...
    for i in range(splits):
        ...
        input_batch = prediction_input_list[
        ...
        save_predicted_images_as_geotiff(
            np_pred_results_iteration, input_batch, raster_output_dir, projection
        )
...
def batch_already_produced(input_batch: list[str], raster_output_dir: str):
    ...
    last_in_batch = Path(input_batch[-1]["image"].file_path).stem + "_prediction.tif"
...
def get_dataset_to_predict_dir(region_name: str, suffix: str = None):
    ...
    prediction_dataset_dir = os.path.join(dataset_dir, "for_prediction")
    ...
    dataset_path_to_predict = os.path.join(prediction_dataset_dir, dataset_name)

    return dataset_path_to_predict


def prepare_dataset_to_predict(
    region_name: str, geom: ogr.Geometry, config: str, num_processes: int = None
):
...
def get_ml_model(
    input_model_name: Model, input_model_subfolder: str = None
) -> keras.Sequential:

    checkpoint_path = get_checkpoint_path(input_model_name, input_model_subfolder)
    ...
    model = keras.models.load_model(checkpoint_path, custom_objects=dependencies)
    return model
...
def get_checkpoint_path(input_model_name: str, input_model_subfolder: str):
    ...
    input_model_path = os.path.join(
        env.get_env_variable("trained_models_directory"), input_model_name
    )

    if input_model_subfolder:
        return os.path.join(input_model_path, input_model_subfolder)

    if not os.path.isdir(input_model_path):
        return input_model_path + ".keras"
...
def get_images_to_predict(
    input_batch: list[dict], img_dims: list[int], download_labels: bool
):
    ...
    for index, sample in enumerate(input_batch, start=0):
        ...
        if "lidar" in sample:
            lidar = sample["lidar"].array.reshape(512, 512, 1)
...
def get_tuples_to_predict(input_batch: list):
    input_tuples = []
    tuple1 = {"name": "image", "dimensions": [512, 512, 3]}
    tuple2 = {"name": "lidar", "dimensions": [512, 512, 1]}
    input_tuples.append(tuple1)
    input_tuples.append(tuple2)

    tupples_to_predict = get_X_tuple(len(input_batch), input_batch, input_tuples)

    return tupples_to_predict


def produce_vector_dataset(
    output_dir: str,
    raster_dir: str,
    config: dict,
    max_batch_size: int,
    ...
    for i in range(num_splits):
        print(
            f"Starting post processing of resulting labels, iteration {i} of {num_splits}, images {i*batch_size} to {i*batch_size+batch_size}"
        )  # image 67400 to 67600
        if i == num_splits:
            # Last run, batch size might be lower
            batch_prediction_paths = predictions_path[
                i * batch_size : (len(predictions_path) - 1)
            ]
        else:
            batch_prediction_paths = predictions_path[
        ...
        if all_predicted_features_dataset:
            save_dataset(
                all_predicted_features_dataset,
                f"raw_predictions_{str(i)}.json",
                output_dir,
                modelname,
                save_to,
            )
...
def polygonize_mask(mask, img, crs):
    ...
    shapes = rasterio.features.shapes(mask, connectivity=4, transform=img.transform)
    ...
    polygons = (
        gp.GeoDataFrame.from_features(geoms, crs=crs)
        if len(geoms) > 0
...
def add_confidence_values(dataset: gp.geodataframe, predictions_path: list[str]):
    ...
    probabilites = zonal_stats(
        dataset.geometry, full_img[0], affine=full_transform, stats="mean", nodata=0
    )
...
def simplify_dataset(dataset: gp.geodataframe):
    ...
    dataset["geometry"] = dataset.simplify(tolerance=0.25)
...
def get_all_predicted_features_dataset(
    predictions_path: list[str], crs: str, region_dir: str = None
):
    ...
    for prediction_path in predictions_path:
        raw_prediction_img = rasterio.open(prediction_path)

        prediction_mask = create_normalized_prediction_mask(raw_prediction_img)
        prediction_polygons = polygonize_mask(prediction_mask, raw_prediction_img, crs)

        predictions = pd.concat([predictions, prediction_polygons], ignore_index=True)
    ...
    merged_predictions["geometry"] = merged_predictions.simplify(tolerance=0.25)
...
def clip_to_polygon(dataset: gp.GeoDataFrame, polygon_file: str, crs: str):
    ...
    dataset = gp.clip(dataset, area_gdf)
    return dataset
...
def merge_connected_geoms(geoms: gp.geodataframe):
    try:
        ...
        dissolved_geoms = dissolved_geoms.explode(index_parts=True).reset_index(
            drop=True
        )
...
def get_label_source_name(
    config: dict, region_name: str = None, is_performance_test: bool = False
):

    labelSourceName = None
    if is_performance_test and ResultRegion.KRISTIANSAND == ResultRegion.from_str(
        region_name
    ):
