...
def predict_image(
    image_path, lidar_path, label_path, datagenerator_config, checkpoint_to_predict_with
):
    model = load_checkpoint_model(checkpoint_to_predict_with)

    input_images = _get_input_images(
        [{"image": image_path, "label": label_path, "lidar": lidar_path}],
...
def evaluate_image(
    image_path, label_path, datagenerator_config, checkpoint_to_predict_with
):
    model = load_checkpoint_model(checkpoint_to_predict_with)
...
def load_checkpoint_model(checkpoint_name: str) -> keras.Sequential:
    ...
    model_fn = os.path.join(checkpoint_path, checkpoint_name + ".keras")
    dependencies = {
...
def checkpoint_exist(checkpoint_name):
    checkpoint_path = env.get_env_variable("trained_models_directory")
    model_path = os.path.join(checkpoint_path, checkpoint_name + ".keras")
