...
from kartai.utils.model_utils import (
    get_ground_truth,
...
def predict_and_evaluate(
    created_datasets_path: str,
    datagenerator_config: str,
    checkpoint_name_to_predict_with: str,
    save_prediction_images: bool = True,
    save_diff_images: bool = True,
    generate_metadata: bool = True,
    dataset_to_evaluate: str = "test",
    batch_size=6,
):
    ...
    with open(
        os.path.join(created_datasets_path, f"{dataset_to_evaluate}_set.json"), "r"
    ) as file:
        input_list_dataset_json = json.load(file)
        input_paths = Tile.tileset_from_json(input_list_dataset_json)

    input_images = _get_input_images(input_paths, datagenerator_config)
    ...
    output_dir = os.path.join(
        env.get_env_variable("prediction_results_directory"),
        checkpoint_name_to_predict_with,
    )
    ...
    if generate_metadata:
        create_metadata_file(
            created_datasets_path, checkpoint_name_to_predict_with, output_dir, results
        )
...
def _save_outputs(
    ...
    if save_prediction_images:
        file_list, _ = save_predicted_images_as_geotiff(
            predictions, input_paths, output_dir, "_val_predict.tif", projection
        )

        gdal.BuildVRT(
            os.path.join(output_dir, "val_predict.vrt"), file_list, addAlpha=True
        )

    if save_diff_images:
        file_list, _ = save_predicted_images_as_geotiff(
            predictions - input_labels,
            input_paths,
            output_dir,
            "_val_diff.tif",
            projection,
        ...
        gdal.BuildVRT(
            os.path.join(output_dir, "val_diff.vrt"), file_list, addAlpha=True
        )


def save_predicted_images_as_geotiff(
    np_predictions,
    ...
    for i in range(len(np_predictions)):
        ...
        prediction_output_dir = (
            input_img_name + ".tif" if suffix else input_img_name + "_prediction.tif"
        )
        ...
        ds = gdal.GetDriverByName("GTiff").Create(
            prediction_output_dir,
            np_predictions.shape[1],
            np_predictions.shape[2],
            1,
            gdal.GDT_Float32,
            ["COMPRESS=LZW", "PREDICTOR=2"],
        )

        transformation = get_transformation(data_samples[i])
...
def create_contour_result(
    raster_path: str, output_dir: str, projection: str, fixed_level_count: list[float]
):
    ...
    prediction_output_dir_geojson = os.path.join(output_dir, "complete_contour.json")
    ...
    gdal.ContourGenerate(
        srcBand=vrt_res.GetRasterBand(1),
        contourInterval=0.0,
        contourBase=1.0,
        fixedLevelCount=fixed_level_count,
        useNoData=0,
        noDataValue=0,
        dstLayer=out_geojson_layer,
        idField=0,
        elevField=1,
    )
...
def create_metadata_file(created_datasets_path, checkpoint_path, output_dir, results):
    ...
    meta = {
        "test dataset path:": str(created_datasets_path),
...
def add_parser(subparser):
    ...
    parser.add_argument(
        ...
        "--dataset_name",
        type=str,
        help="Name for training dataset",
        required=True,
    )
    parser.add_argument(
        "-cn",
        ...
        help="Name for checkpoint file to use for prediction",
        required=True,
    )
    parser.add_argument(
        "-s",
        "--save_prediction_images",
        action="store_true",
        help="Wether or not to save predictions as images",
        default=True,
    )
    parser.add_argument(
        "-diff",
...
def main(args):
    ...
    created_datasets_dir = os.path.join(
        env.get_env_variable("created_datasets_directory"), args.dataset_name
    )
    checkpoint_path = os.path.join(
        env.get_env_variable("trained_models_directory"), args.checkpoint_name + ".h5"
    )
    ...
    predict_and_evaluate(
        ...
        args.save_prediction_images,
        args.save_difference_images,
        args.batch_size,
    )
